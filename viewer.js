...
(() => {
  ...
  function renderPisteView() {
    ...
    const pisteCount = state.pisteCount || 0;
    if (pisteCount === 0 || !state.pools || state.pools.length === 0) {
      container.className = 'empty-state';
      container.innerHTML = 'ピスト情報なし';
      return;
    }
    container.className = '';
    const pistes = {};
        ...
        <div style="margin-bottom:18px">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:6px">
            ${p.matches.map((m, mi) => {
              ...
              const aWin = m.completed && m.scoreA > m.scoreB ? 'font-weight:bold;color:var(--color-primary)' : '';
              const bWin = m.completed && m.scoreB > m.scoreA ? 'font-weight:bold;color:var(--color-primary)' : '';
              return `
                <div style="padding:6px 8px;border:1px solid var(--color-border);border-radius:4px;background:${bg};font-size:0.85rem">
                  ...
                  <span style="${aWin}">${escapeHtml(m.fencerAName)}</span>
                  <strong>${m.scoreA}</strong>
                  <span style="color:#6b7280">-</span>
                  <strong>${m.scoreB}</strong>
                  <span style="${bWin}">${escapeHtml(m.fencerBName)}</span>
                </div>
              `;
            }).join('')}
          </div>
